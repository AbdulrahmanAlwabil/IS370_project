...
ctk.set_default_color_theme("dark-blue")  # Available themes: "blue" (default), "green", "dark-blue"
...
class LoginSignupApp(ctk.CTk):
    def __init__(self):
        ...
        # Signup button
        self.signup_button = ctk.CTkButton(self.frame, text="Signup", command=self.signup)
...
class MessengerApp(ctk.CTk):
    def __init__(self):
        ...
        # Title for contacts
        self.contacts_label = ctk.CTkLabel(self.left_frame, text="Contacts", font=("Helvetica", 16))
        self.contacts_label.grid(row=0, column=0, padx=10, pady=5)

        # Scrollable frame for the list of contacts
        self.contacts_frame = ctk.CTkScrollableFrame(self.left_frame, width=180, height=500)
        self.contacts_frame.grid(row=1, column=0, padx=10, pady=5, sticky="nswe")

        # Dummy list of contacts
        self.contacts = ["Alice", "Bob", "Charlie", "David"]
        for contact in self.contacts:
            contact_btn = ctk.CTkButton(self.contacts_frame, text=contact,
        ...
        self.chat_title_label = ctk.CTkLabel(self.right_frame, text="Select a contact to chat", font=("Helvetica", 16))
        ...
        self.message_entry = ctk.CTkEntry(self.bottom_frame, placeholder_text="Type your message here...")
        self.message_entry.grid(row=0, column=0, padx=5, pady=5, sticky="ew", columnspan=2)

        # Button to send the message
        self.send_button = ctk.CTkButton(self.bottom_frame, text="Send", command=self.send_message)
        self.send_button.grid(row=1, column=0, padx=5, pady=5, sticky="ew")

        # Button to attach an image or video
        self.attach_button = ctk.CTkButton(self.bottom_frame, text="Attach", command=self.attach_file)
    ...
    def add_message(self, message, sender="other"):
        ...
        if sender == "me":
            ...
            message_label = ctk.CTkLabel(message_frame, text=message, fg_color="#284b9e", corner_radius=5, anchor="e")
            message_label.pack(side="right", padx=10)
        else:
            # Other user's messages are left-aligned
            message_label = ctk.CTkLabel(message_frame, text=message, fg_color="#1c8018", corner_radius=5, anchor="w")
